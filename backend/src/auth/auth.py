...
'''
@INPUTS
    token: a json web token (string) - Auth0 token with key id (kid)

Verifies the token using Auth0 /.well-known/jwks.json
Decodes the payload from the token.
Validates the claims, and returns the decoded payload
'''
...
Uses the check_permissions method validate claims and check
